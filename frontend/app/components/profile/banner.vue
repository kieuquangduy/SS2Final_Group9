<template>
    <div class="w-full rounded-lg block h-full">
        <NuxtImg :src=PROFILEBANNER class="w-full h-40 object-cover rounded-t-lg" format="webp" quality="70" />
        <CommonPageSection
...
:src="PROFILEIMAGE"
            ...
            <div class="lg:ml-auto flex gap-2">
                <UButton
v-for="(action, id) in PROFILEACTIONS" :key="id" :label="action.label" :color="action.color"
                    :variant="action.variant" :to="action.to" class="h-10 cursor-pointer px-6" />
            </div>
        </CommonPageSection>
    </div>
</template>
