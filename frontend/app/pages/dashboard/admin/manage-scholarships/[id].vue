...
import { useScholarshipEdit } from '~/composables/scholarship/useScholarshipEdit'
...
const { data } = await useScholarshipDetail(id as string)
const selectedOrganizers = ref<{ organizer: Tables<'organizer_list_view'>, host: boolean }[]>([
  ...data.value!.organizers!.map(org => ({
    organizer: org,
    host: org.host,
  })),
])
...
const formState = reactive({
  ...
  tier: data.value?.tier ?? 'VENUE',
  deadline: data.value?.deadline ?? currentDay.toString(),
...
const tempBanner = ref<string>(data.value?.banner_url ?? '')
const tempIcon = ref<string>(data.value?.icon_url ?? '')
...
const { isLoading, editScholarship } = await useScholarshipEdit()
